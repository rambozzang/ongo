 ...
 *
 * 영상 생성·업로드·게시 같은 주요 사용자 쓰기 경로와 예약/반복 게시 경로에서
 * 공통으로 사용한다. 새 사용자 쓰기 경로를 추가할 때도 이 관문을 먼저 통과시켜야
 * 삭제 동결 중인 계정에 외부 상태나 내부 데이터가 추가되지 않는다.
 *
 * HTTP 필터만으로는 충분하지 않다. 예약 배치와 내부 서비스 호출은 필터를 지나지
 * 않기 때문에, 진입 경로와 무관하게 검사할 수 있는 도메인 포트로 유지한다.
 *
