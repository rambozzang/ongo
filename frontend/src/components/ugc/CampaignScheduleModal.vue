<template>
  <BaseModal v-model="isOpen" :title="$t('ugc.scheduleModalTitle')" max-width="lg">
    <p class="mb-5 text-body text-gray-500 dark:text-gray-400">{{ $t('ugc.scheduleModalDescription') }}</p>
    ...
    <div class="space-y-3">
      <section class="rounded-xl border border-gray-200 bg-gray-50/70 p-4 dark:border-gray-700 dark:bg-gray-800/50">
        <div class="mb-3 flex items-start gap-3">
          ...
          <div>
            <h4 class="text-body font-semibold text-gray-900 dark:text-gray-100">{{ $t('ugc.startAt') }}</h4>
            <p class="mt-0.5 text-body-xs text-gray-500 dark:text-gray-400">{{ $t('ugc.startAtHint') }}</p>
          </div>
        </div>
        <div class="grid grid-cols-1 gap-3 mobile:grid-cols-2">
          <label class="block">
            <span class="mb-1 block text-caption text-gray-600 dark:text-gray-300">{{ $t('ugc.date') }}</span>
            <input v-model="startDate" type="date" class="input-field" />
          </label>
          <label class="block">
            <span class="mb-1 block text-caption text-gray-600 dark:text-gray-300">{{ $t('ugc.time') }}</span>
      ...
      <section class="rounded-xl border border-gray-200 bg-gray-50/70 p-4 dark:border-gray-700 dark:bg-gray-800/50">
        <div class="mb-3 flex items-start gap-3">
          <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-primary-100 text-primary-700 dark:bg-primary-900 dark:text-primary-300">
            <FlagIcon class="h-5 w-5" />
          </div>
          <div>
            <h4 class="text-body font-semibold text-gray-900 dark:text-gray-100">{{ $t('ugc.endAt') }}</h4>
            <p class="mt-0.5 text-body-xs text-gray-500 dark:text-gray-400">{{ $t('ugc.endAtHint') }}</p>
          </div>
        </div>
        <div class="grid grid-cols-1 gap-3 mobile:grid-cols-2">
          <label class="block">
    ...
    <p v-if="errorMessage" class="mt-4 rounded-lg bg-error-subtle px-3 py-2 text-body text-error-strong" role="alert">
      {{ errorMessage }}
    </p>
