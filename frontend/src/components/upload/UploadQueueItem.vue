...
const statusBadgeClass = computed(() => {
  const map: Record<string, string> = {
    ...
    uploading: 'bg-info-subtle text-info-strong',
...
<template>
  <div
    ...
    :class="{
      ...
      'border-l-4 border-l-warning': item.status === 'paused',
      'border-l-4 border-l-success': item.status === 'completed',
      'border-l-4 border-l-error': item.status === 'failed',
    }"
  >
    <div class="flex items-start gap-3">
      <!-- Index number + File icon -->
      <div class="flex flex-col items-center gap-1">
        <span class="text-caption text-gray-400 dark:text-gray-500">{{ index + 1 }}</span>
      ...
      <div class="min-w-0 flex-1">
        ...
        <div class="flex items-center justify-between gap-2">
          <p
            class="truncate text-body font-medium text-gray-900 dark:text-gray-100"
            :title="item.fileName"
          >
            {{ truncatedName }}
          </p>
          <span
            class="flex-shrink-0 rounded-full px-2 py-0.5 text-caption"
        ...
        <div class="mt-3 flex items-center gap-1">
          ...
          <button
            ...
            class="inline-flex items-center gap-1 rounded-md px-2.5 py-1 text-caption text-warning-strong transition-colors hover:bg-warning-subtle"
            title="일시정지"
          ...
          <button
            ...
            class="inline-flex items-center gap-1 rounded-md px-2.5 py-1 text-caption text-primary-700 transition-colors hover:bg-primary-100 dark:text-primary-400 dark:hover:bg-primary-900/30"
            title="재개"
          ...
          <button
            v-if="canMoveUp"
            class="inline-flex items-center gap-0.5 rounded-md px-2 py-1 text-caption text-gray-600 transition-colors hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-700"
            title="위로 이동"
          ...
          <button
            v-if="canRemove"
            class="inline-flex items-center gap-1 rounded-md px-2.5 py-1 text-caption text-error-strong transition-colors hover:bg-error-subtle"
            title="제거"
