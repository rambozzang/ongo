    ...
    content: [
      ...
      },
      {
        ...
        items: [
          ...
          '기본 흐름에서는 렌더 스펙이 준비된 클립의 "서버에서 렌더"를 누르면 완성 영상이 자동으로 생성·저장·연결됩니다. 완료 후 미리보기로 확인하고 예약을 확정하면 선택한 플랫폼에 게시됩니다.',
          '서버 렌더를 사용할 수 없는 환경에서는 render.sh 산출물을 직접 업로드한 뒤 "완성 영상 연결"을 보완 경로로 사용할 수 있습니다.',
    ...
    content: [
      ...
      },
      {
        ...
        items: [
          ...
          'In the normal flow, click "Render on server" on a clip with a render spec. The finished video is rendered, stored, and attached automatically; preview it and confirm the schedule to publish it to the selected platforms.',
          'If server rendering is unavailable, you can upload a video made with render.sh and use "Attach rendered video" as a fallback.',
