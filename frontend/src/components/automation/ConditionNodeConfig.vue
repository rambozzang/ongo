...
<template>
  <div class="space-y-4">
    ...
    <!-- AND/OR Toggle -->
    <div>
      <label class="block text-caption text-gray-600 dark:text-gray-400 mb-1">그룹 연산자</label>
      <div class="flex gap-2">
        <button
          v-for="type in (['AND', 'OR'] as const)"
          :key="type"
          class="flex-1 py-2 rounded-lg text-body font-medium transition-colors"
          :class="
            groupType === type
              ? type === 'AND'
    ...
    <div class="flex items-center gap-2">
      ...
      <span class="text-body-xs text-gray-600 dark:text-gray-400">SpEL 표현식 사용</span>
    </div>

    <!-- SpEL Expression Mode -->
    <div v-if="useSpel" class="space-y-3">
      <div>
        <label class="block text-caption text-gray-600 dark:text-gray-400 mb-1">SpEL 표현식</label>
        <textarea
          :value="expression ?? ''"
          rows="3"
          class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 px-3 py-2 text-body font-mono"
          placeholder="#video.views > 10000 AND #video.platform == 'YOUTUBE'"
          @input="expression = ($event.target as HTMLTextAreaElement).value"
        />
      </div>
      <div class="text-body-xs text-gray-500 dark:text-gray-400 space-y-1">
    ...
    <div v-else class="space-y-3">
      <div>
        <label class="block text-caption text-gray-600 dark:text-gray-400 mb-1">필드</label>
        <select
          :value="field ?? ''"
      ...
      <div>
        <label class="block text-caption text-gray-600 dark:text-gray-400 mb-1">연산자</label>
        <select
      ...
      <div>
        <label class="block text-caption text-gray-600 dark:text-gray-400 mb-1">값</label>
        <input
          type="text"
          :value="value ?? ''"
          class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 px-3 py-2 text-body"
