...
<template>
  <div class="space-y-3">
    <div class="flex items-center justify-between">
      <h3 class="text-body font-semibold text-gray-900 dark:text-gray-100">실행 이력</h3>
      <button
    ...
    <div v-if="loading" class="text-center py-6 text-body text-gray-500">
      로딩 중...
    </div>

    <div v-else-if="executions.length === 0" class="text-center py-6 text-body text-gray-400">
      실행 이력이 없습니다
    </div>
    ...
    <div v-else class="space-y-2">
      ...
      >
        ...
        >
          ...
          <div class="flex items-center gap-2">
            <span class="text-body-xs text-gray-500">
        ...
        <div v-if="expandedId === exec.id" class="border-t border-gray-200 dark:border-gray-700 p-3 space-y-2 bg-gray-50 dark:bg-gray-800/50">
          <div v-if="exec.errorMessage" class="text-body-xs text-error-strong bg-error-subtle rounded p-2">
            {{ exec.errorMessage }}
          </div>
          <div v-for="(result, i) in exec.actionResults" :key="i" class="flex items-center gap-2 text-body-xs">
            <span
          ...
          <div v-if="exec.completedAt" class="text-body-xs text-gray-400 pt-1">
