<template>
  <div class="youtube-preview">
    <div class="mb-3 flex items-center justify-between">
      <span class="text-body font-medium text-gray-700 dark:text-gray-300">{{ t('preview.howItLooks') }}</span>
      <div class="flex gap-1 rounded-lg bg-gray-100 p-0.5 dark:bg-gray-800">
        <button
          :class="[
            'rounded-md px-2.5 py-1 text-body-xs font-medium transition-colors',
            viewMode === 'feed'
    ...
    <div v-if="viewMode === 'feed'" class="overflow-hidden rounded-lg bg-white shadow-lg dark:bg-gray-900">
      ...
      <div class="relative aspect-video bg-black">
        ...
        <!-- Duration badge -->
        <div class="absolute bottom-2 right-2 rounded bg-black/80 px-1.5 py-0.5 text-body-xs font-medium text-white">
          0:00
        </div>
      </div>
        ...
        <div class="flex gap-3">
          ...
          <div class="mt-0.5 flex-shrink-0">
            ...
            <div class="flex h-9 w-9 items-center justify-center rounded-full bg-red-600">
              <span class="text-body font-bold text-white">{{ channelInitial }}</span>
            </div>
          </div>
          ...
          <div class="min-w-0 flex-1">
            <!-- Title -->
            <h3
              class="mb-1 line-clamp-2 text-body font-medium leading-5 text-gray-900 dark:text-gray-100"
              :title="title"
            >
            ...
            <div class="mb-1 flex items-center gap-1.5">
              ...
              <span
                ...
                class="rounded bg-error-subtle px-1.5 py-0.5 text-[10px] font-semibold text-error-strong"
              >
            ...
            <!-- Channel & Meta -->
            <div class="text-body-xs text-gray-600 dark:text-gray-400">
      ...
      <div v-if="description" class="border-t border-gray-100 px-3 pb-3 dark:border-gray-800">
        <div class="mt-3">
          <p class="line-clamp-3 text-body-xs leading-5 text-gray-700 dark:text-gray-300 whitespace-pre-line">{{ description }}</p>
          <button class="mt-1 text-body-xs font-medium text-gray-600 dark:text-gray-400">더보기</button>
        </div>
      </div>
      ...
      <div v-if="tags && tags.length > 0" class="border-t border-gray-100 px-3 pb-3 pt-2 dark:border-gray-800">
        <div class="flex flex-wrap gap-1.5">
          <span
            ...
            :key="idx"
            class="text-body-xs font-medium text-info-strong"
          >
            #{{ tag }}
          </span>
          <span
      ...
      <div class="flex items-center justify-between border-t border-gray-100 px-1 py-1 dark:border-gray-800">
        <button class="flex flex-1 items-center justify-center gap-1.5 rounded-full py-2 text-gray-700 dark:text-gray-300">
          ...
          <span class="text-body-xs font-medium">좋아요</span>
        </button>
        <button class="flex flex-1 items-center justify-center gap-1.5 rounded-full py-2 text-gray-700 dark:text-gray-300">
          <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <button class="flex flex-1 items-center justify-center gap-1.5 rounded-full py-2 text-gray-700 dark:text-gray-300">
          ...
          <span class="text-body-xs font-medium">공유</span>
        </button>
        <button class="flex flex-1 items-center justify-center gap-1.5 rounded-full py-2 text-gray-700 dark:text-gray-300">
          <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z"/>
          </svg>
          <span class="text-body-xs font-medium">저장</span>
        </button>
      </div>
    </div>
    ...
    <div v-else class="overflow-hidden rounded-lg bg-white shadow-lg dark:bg-gray-900">
      <div class="flex gap-3 p-3">
        ...
        <div class="min-w-0 flex-1">
          ...
          <!-- Channel & Meta -->
          <div class="mb-1 flex items-center gap-1.5 text-body-xs text-gray-600 dark:text-gray-400">
          ...
          <div v-if="tags && tags.length > 0" class="mt-1.5 flex flex-wrap gap-1">
            <span
              ...
              :key="idx"
              class="text-[11px] font-medium text-info-strong"
            >
              #{{ tag }}
            </span>
            <span
...
const titleCountColor = computed(() => {
  if (titleLength.value > 100) return 'text-error-strong font-semibold'
  if (titleLength.value >= 80) return 'text-warning-strong'
