  ...
  <div class="max-w-4xl">
    ...
    <div v-if="inviteUrl" class="card mb-4 flex flex-col gap-2 mobile:flex-row mobile:items-center">
      <div class="min-w-0 flex-1">
        <p class="text-body-xs text-gray-400">{{ $t('ugc.inviteLink') }}</p>
        <p class="truncate font-mono text-body text-gray-900 dark:text-gray-100">{{ inviteUrl }}</p>
      </div>
    ...
    <div v-else-if="applications.length === 0" class="card py-16 text-center text-body text-gray-500 dark:text-gray-400">
      ...
      <div v-for="a in applications" :key="a.id" class="card flex items-start justify-between gap-4">
        <div class="min-w-0 flex-1">
          <div class="flex items-center gap-2">
            ...
            <span :class="['rounded-full px-2 py-0.5 text-caption', appStatusClass(a.status)]">
              {{ $t(`ugc.appStatus.${a.status}`) }}
            </span>
          </div>
          <p v-if="a.message" class="mt-1 text-body text-gray-600 dark:text-gray-300">{{ a.message }}</p>
          <a v-if="a.portfolioUrl" :href="a.portfolioUrl" target="_blank" rel="noopener noreferrer" class="mt-1 inline-block text-body-xs text-primary-600 hover:underline">
            {{ a.portfolioUrl }}
          </a>
          <p class="mt-1 text-body-xs text-gray-400">{{ a.createdAt?.slice(0, 10) }}</p>
        </div>
        <div v-if="a.status === 'APPLIED'" class="flex shrink-0 gap-2">
          <button class="btn-secondary text-body-xs" :disabled="acting === a.id" @click="decide(a.id, 'reject')">{{ $t('ugc.reject') }}</button>
          <button class="btn-primary text-body-xs" :disabled="acting === a.id" @click="decide(a.id, 'accept')">{{ $t('ugc.accept') }}</button>
        </div>
      </div>
    </div>
