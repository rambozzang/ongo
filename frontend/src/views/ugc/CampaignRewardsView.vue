    ...
    <template v-else>
      ...
      <div class="card mb-4">
        <div class="mb-2 flex items-center justify-between">
          <h3 class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ $t('ugc.performance') }}</h3>
          <span class="text-xs text-gray-400">{{ $t('ugc.lastSynced') }}: {{ analytics?.lastSyncedAt?.slice(0, 16)?.replace('T', ' ') || '-' }}</span>
        </div>
        <div class="grid grid-cols-2 gap-4 mobile:grid-cols-4">
          <div><p class="text-xs text-gray-400">{{ $t('ugc.views') }}</p><p class="mt-0.5 text-lg font-semibold text-gray-900 dark:text-gray-100">{{ (analytics?.totalViews ?? 0).toLocaleString() }}</p></div>
          <div><p class="text-xs text-gray-400">{{ $t('ugc.likes') }}</p><p class="mt-0.5 text-lg font-semibold text-gray-900 dark:text-gray-100">{{ (analytics?.totalLikes ?? 0).toLocaleString() }}</p></div>
          <div><p class="text-xs text-gray-400">{{ $t('ugc.comments') }}</p><p class="mt-0.5 text-lg font-semibold text-gray-900 dark:text-gray-100">{{ (analytics?.totalComments ?? 0).toLocaleString() }}</p></div>
          <div><p class="text-xs text-gray-400">{{ $t('ugc.shares') }}</p><p class="mt-0.5 text-lg font-semibold text-gray-900 dark:text-gray-100">{{ (analytics?.totalShares ?? 0).toLocaleString() }}</p></div>
        </div>
      </div>

      <!-- Budget -->
      <div class="card mb-4 grid grid-cols-3 gap-4 text-center">
        <div><p class="text-xs text-gray-400">{{ $t('ugc.budget') }}</p><p class="mt-0.5 font-semibold text-gray-900 dark:text-gray-100">{{ (rewards?.totalBudget ?? 0).toLocaleString() }}</p></div>
        <div><p class="text-xs text-gray-400">{{ $t('ugc.confirmed') }}</p><p class="mt-0.5 font-semibold text-gray-900 dark:text-gray-100">{{ (rewards?.settledTotal ?? 0).toLocaleString() }}</p></div>
        <div><p class="text-xs text-gray-400">{{ $t('ugc.remaining') }}</p><p :class="['mt-0.5 font-semibold', (rewards?.remaining ?? 0) < 0 ? 'text-red-600' : 'text-gray-900 dark:text-gray-100']">{{ (rewards?.remaining ?? 0).toLocaleString() }}</p></div>
      </div>

      <!-- Participants rewards -->
      <div v-if="!rewards || rewards.items.length === 0" class="card py-16 text-center text-sm text-gray-500 dark:text-gray-400">
      ...
      <div v-else class="space-y-3">
        <div v-for="item in rewards.items" :key="item.participantId" class="card">
          <div class="flex items-center justify-between gap-2">
            ...
            <span :class="['rounded-full px-2 py-0.5 text-xs font-medium', rewardStatusClass(item.status)]">{{ $t(`ugc.rewardStatus.${item.status}`) }}</span>
          </div>
          <div class="mt-3 grid grid-cols-2 gap-3 mobile:grid-cols-4">
            <div>
              <label class="text-xs text-gray-400">{{ $t('ugc.baseAmount') }}</label>
              <input v-model.number="item.baseAmount" type="number" min="0" class="input-field mt-1" :disabled="item.status !== 'DRAFT'" />
            </div>
            <div>
              <label class="text-xs text-gray-400">{{ $t('ugc.bonusAmount') }}</label>
              <input v-model.number="item.bonusAmount" type="number" min="0" class="input-field mt-1" :disabled="item.status !== 'DRAFT'" />
            </div>
            <div>
              <label class="text-xs text-gray-400">{{ $t('ugc.totalAmount') }}</label>
