  ...
  <div class="max-w-xl py-8">
    ...
    <div v-else-if="campaign" class="card space-y-5">
      <div>
        ...
        <h1 class="mt-2 text-2xl font-bold text-gray-900 dark:text-gray-100">{{ campaign.name }}</h1>
        <p v-if="campaign.description" class="mt-2 text-sm text-gray-600 dark:text-gray-300">{{ campaign.description }}</p>
      </div>

      <dl class="grid grid-cols-2 gap-4 border-y border-gray-100 py-4 text-sm dark:border-gray-700">
        <div>
          <dt class="text-xs text-gray-400">{{ $t('ugc.reward') }}</dt>
        ...
        <div>
          <dt class="text-xs text-gray-400">{{ $t('ugc.period') }}</dt>
      ...
      <div v-if="campaign.playbookTitle">
        <h2 class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ $t('ugc.playbook') }}</h2>
        <p class="mt-1 font-medium text-gray-800 dark:text-gray-200">{{ campaign.playbookTitle }}</p>
        <p v-if="campaign.playbookSummary" class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ campaign.playbookSummary }}</p>
      </div>

      <!-- Already applied -->
      <div v-if="campaign.alreadyApplied" class="rounded-lg bg-green-50 p-4 text-center text-sm text-green-700 dark:bg-green-900/20 dark:text-green-300">
      ...
      <template v-else>
        <div>
          <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">{{ $t('ugc.applyMessage') }}</label>
          <textarea v-model="message" rows="3" class="input-field" :placeholder="$t('ugc.applyMessagePlaceholder')" />
        </div>
        <div>
          <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">{{ $t('ugc.portfolioUrl') }}</label>
