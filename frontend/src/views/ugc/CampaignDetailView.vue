<template>
  <div class="min-h-full max-w-3xl space-y-5 py-5 text-content">
    <button class="mb-4 inline-flex items-center gap-1 text-body text-gray-500 hover:text-gray-700 dark:text-gray-400" @click="router.push('/ugc/campaigns')">
    ...
    <template v-else-if="campaign">
      <PageHeader :title="campaign.name" :description="campaign.description || $t('ugc.noDescription')">
        <template #title-suffix>
          <span :class="['rounded-full px-2.5 py-0.5 text-caption', statusClass(campaign.status)]">
      ...
      <div class="page-grid page-grid--metrics card mb-4">
        <div>
          <p class="text-body-xs text-gray-400">{{ $t('ugc.fieldObjective') }}</p>
          <p class="mt-0.5 text-body font-medium text-gray-900 dark:text-gray-100">{{ campaign.objective }}</p>
        </div>
        <div>
          <p class="text-body-xs text-gray-400">{{ $t('ugc.budget') }}</p>
          <p class="mt-0.5 text-body font-medium text-gray-900 dark:text-gray-100">{{ formatMoney(campaign.totalBudget, campaign.currency) }}</p>
        </div>
        <div>
          <p class="text-body-xs text-gray-400">{{ $t('ugc.reward') }}</p>
          <p class="mt-0.5 text-body font-medium text-gray-900 dark:text-gray-100">{{ formatMoney(campaign.fixedRewardPerCreator, campaign.currency) }}</p>
        </div>
        <div>
          <p class="text-body-xs text-gray-400">{{ $t('ugc.period') }}</p>
          <p class="mt-0.5 text-body font-medium text-gray-900 dark:text-gray-100">{{ formatPeriod(campaign.startAt, campaign.endAt) }}</p>
        </div>
      </div>
      ...
      <div class="card">
        <div class="mb-3 flex items-center justify-between">
          ...
          <button v-if="!isTerminal" class="btn-secondary text-body-xs" @click="router.push(`/ugc/campaigns/${campaign.id}/edit`)">
            {{ $t('ugc.edit') }}
          </button>
        </div>
        <div v-if="playbook">
          <p class="font-medium text-gray-900 dark:text-gray-100">{{ playbook.title }}</p>
          <p v-if="playbook.summary" class="mt-1 text-body text-gray-500 dark:text-gray-400">{{ playbook.summary }}</p>
          <span class="mt-2 inline-block rounded bg-gray-100 px-2 py-0.5 text-body-xs text-gray-600 dark:bg-gray-700 dark:text-gray-300">{{ playbook.contentType }}</span>
          <ol v-if="playbook.steps.length" class="mt-3 space-y-2">
            <li v-for="(s, i) in playbook.steps" :key="i" class="rounded-lg border border-gray-100 p-3 text-body dark:border-gray-700">
        ...
        <p v-else class="text-body text-gray-400">{{ $t('ugc.noPlaybook') }}</p>
      </div>
    </template>
