...
abort_migration_needed() {
    ...
    error ""
    error "조치: V94~V118 를 순서대로 먼저 적용한 뒤 다시 배포하세요."
...
if [ "$APPLIED" != "1" ]; then
    abort_migration_needed "V$REQUIRED_FLYWAY_VERSION 이 적용되지 않았습니다(현재: ${CURRENT_VERSION:-알 수 없음}). V94~V118 선행 적용이 필요합니다."
fi
...
# V114 도 컬럼이 아니라 enum 값을 더한다. 같은 이유로 위 컬럼 검사에 걸리지 않는다.
#
# 이 값들이 없으면 **문제가 생겼을 때 알려주는 장치**만 골라서 죽는다.
#   - CHANNEL_TOKEN_EXPIRED: 채널이 EXPIRED 로 바뀐 뒤 알림 저장이 실패한다. 채널은
#     끊기고 사용자는 영원히 모르며, 예외가 forEach 를 뚫고 나가 그 주기의 나머지
#     채널은 토큰 갱신 시도조차 못 한다.
#   - REVENUE_ALERT: 수익 알림 저장이 실패한다.
#   - SUSPENDED: 관리자의 구독 정지가 실패한다.
while IFS=: read -r enum_type enum_label reason; do
    [ -n "$enum_type" ] || continue
    FOUND="$(query "SELECT 1 FROM pg_enum e JOIN pg_type t ON t.oid = e.enumtypid WHERE t.typname = '$enum_type' AND e.enumlabel = '$enum_label' LIMIT 1")"
    if [ "$FOUND" != "1" ]; then
        abort_migration_needed "$enum_type enum 에 '$enum_label' 값이 없습니다(V114 미적용). $reason"
    fi
done <<'ENUM_CHECKS'
