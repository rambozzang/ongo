...
#### 보안 불변식 — payload 의 `userId` 를 단독 근거로 쓰지 않는다 (codex)

`SYSTEM_RECONCILIATION` 은 **동결을 뚫는 권한**이다. 그 근거를 payload 에서 읽은
값 하나에 두면, 위조되거나 오래된 payload 로 **임의 사용자의 동결을 우회**할 수 있다.
재시도 경로는 저장된 payload 를 다시 처리하므로 특히 그렇다.

순서를 못 박는다.

1. 서명·이벤트 진위를 먼저 검증한다
2. 게이트웨이 재조회 또는 결제 DB 조회로 **payment ↔ user 연결을 확정**한다
3. 그렇게 확정된 `userId` 로만 우회를 호출한다

`custom_data.user_id` 는 힌트이지 권한 근거가 아니다.

다음은 전부 **fail-closed 이면서 재시도 가능한 실패**로 남긴다. 조용히 통과시키지 않는다.
- payload 해석 실패
- payment ↔ user 연결 불일치
- 재조회 실패
...
### 2.2.3 전역 감사 호출은 로그가 아니라 구조화 기록이어야 한다 (codex)

(2) 전역 경로가 남기는 것은 `log.info` 한 줄이 아니다. **등록된 경로명 / 근거 /
결과(outcome)** 를 구조화해 남긴다. 문자열 로그로 두면 형식이 흔들리고 집계가 안 되며,
"기록했다"와 "기록이 쓸모 있다"가 갈린다.

IT 로 검증할 것: 감사 호출이 실제로 등록된 path 와 rationale, outcome 을 담는가.
단순히 호출됐다는 것만 확인하면 부족하다.
