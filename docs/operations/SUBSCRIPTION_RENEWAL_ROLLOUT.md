...
### 2-5-1. 꺼져 있던 동안 카드를 등록한 고객은 **자동 결제에 동의하지 않았습니다**

2026-09-23부터 결제 화면의 동의 문구는 이 토글을 따릅니다(`GET /subscriptions/billing-policy`).
꺼진 동안 결제한 고객은 "지금은 자동으로 결제되지 않습니다 … 자동 결제를 시작하게 되면 미리
알려 드립니다" 에 동의했습니다. 빌링키는 저장돼 있지만 **그 키로 청구할 동의는 받지 않았습니다.**

- [ ] 켜기 전에 이 고객들에게 자동 결제 시작을 알리고 동의를 다시 받을 것
- [ ] 동의하지 않은 고객은 갱신 대상에서 빼거나(빌링키 제거) 기간 만료로 두기

같은 이유로, 꺼져 있는 동안에는 `BillingScheduler` 가 기간이 끝난 유료 ACTIVE 구독을 Free 로
내리고 3일 전에 예고 알림을 보냅니다. 켜면 이 두 단계는 멈추고 갱신 청구가 대신합니다.
...
끄면 결제 화면 문구도 즉시 "자동 결제 없음" 으로 바뀌고, 다음 새벽 배치부터 기간이 끝난
ACTIVE 구독은 Free 로 내려갑니다(청구하지 않으므로). 끄기 전에 고객 공지를 먼저 하세요.
